import App, { Maths, Vec3, Colour }	from "../fungi/App.js";
...
class GizmoMove{
	...
	static init( size=null, init_func=null, priority=700 ){
		...
		App.ecs.components.reg( GizmoMove );
		App.ecs.systems.reg( GizmoMoveSys, priority );

		if( size != null){
			App.events.reg( "gizmo_move", size, true, init_func );
			EVT_ON = true;
		}
	}
	...
	*/
	...
	static new_entity(){
		...
		e.gizmo = App.ecs.add_com( e.id, "GizmoMove" ).init();	
		return e;
	}
	...
	init( inc_plane=false ){
		this.node = App.ecs.get_com( this._entity_id, "Node" );
		
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		let draw	= App.ecs.get_com( this._entity_id, "Draw" );
		let mat		= App.shader.new_material( "GizmoMove" );
		draw.add( geo_lines(), mat, App.mesh.LINE );
	...
	is_hit( ray ){
		...
		if( ray_to_bound( ray, this.node.local, this.bound ) ){
			let info = {};
			if( ray_to_axis( ray, this.node.local, info ) ){
				//console.log( info );
				this.drag_begin( this.node.local.pos, info.axis, info.hit_point );
				return true;
			}
		}
		return false;
	...
	hide(){ App.ecs.set_entity_active( this._entity_id, false ); return this; }
	show( pos=null ){
		if( pos ) this.node.set_pos( pos );	// Move Gizmo to starting position.
		App.ecs.set_entity_active( this._entity_id, true );
		return this;
	}
	...
	drag_move( e ){
		...
		this.node.set_pos( pnts[ 1 ].add( this.drag_offset ) );

		// Pass Drag Value
		if( EVT_ON ) App.events.emit( "gizmo_move", pnts[1] );
	}
...
function GizmoMoveSys( ecs ){
	...
	for( c of ary ){		
		...
		if( cam.updated || c.is_drag ){
			eye_dir.from_sub( cam.local.pos, c.node.local.pos );
			eye_len	= eye_dir.len();
...
function geo_lines(){
	...
	return App.mesh.from_data( "GizmoMoveTranLine", new Float32Array( vert ), 4 );
}
/*
function geo_planes(){
	let w		= 0.3,
...
function LoadShader(){
	App.shader
		.new( "GizmoMove", v_src, f_src, [
			{ name:"color_ary", type:"vec3", value:Colour.rgb_array("#ff0000", "#00ff00", "#0000ff") }
