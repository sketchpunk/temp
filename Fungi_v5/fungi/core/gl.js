...
class gl{
	static init( canvas ){
		...
		// canvas.getContext( 'webgl2', { antialias: false, xrCompatible:true } );
	...
	////////////////////////////////////////////////////////
		static clear(){
			this.ctx.clear( gl.ctx.COLOR_BUFFER_BIT | gl.ctx.DEPTH_BUFFER_BIT );
			return this;
		}
	...
	////////////////////////////////////////////////////////
		...
		static create_shader( vShaderSrc, fShaderSrc, doValidate = true, transFeedbackVars = null, transFeedbackInterleaved = false ){
			let vShader		= gl.compile_shader(vShaderSrc, gl.ctx.VERTEX_SHADER);
			if(!vShader)	return null;

			let fShader		= gl.compile_shader(fShaderSrc, gl.ctx.FRAGMENT_SHADER);
			if(!fShader){	ctx.deleteShader(vShader); return null; }

			return gl.create_shader_program( vShader, fShader, doValidate, transFeedbackVars, transFeedbackInterleaved );
		}
		...
		static create_shader_program(vShader, fShader, doValidate = true, transFeedbackVars = null, transFeedbackInterleaved = false ){
